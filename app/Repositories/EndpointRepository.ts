...
export default class EndpointRepository {
  ...
  public async getAccessDetails(data: any) {
    ...
    const roleKeys = [
      ...new Set(assignedPermissions.map((item) => item.roleKey)),
    ];
    ...
    const activeRoleKeys = new Set(
      filteredAssignedPermissions.map((item) => item.roleKey)
    );
    ...
    return {
      ...
      roles: [
        ...new Set(filteredAssignedPermissions.map((item) => item.role.key)),
      ],
