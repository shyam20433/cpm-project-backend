...
export default class AssignedEndpointsController {
  public async getAssignedEndpoints({
    request,
    response,
  }: HttpContextContract) {
    try {
      ...
      const assignedEndpoints =
        await AssignedEndpointRepository.getAssignedEndpoints(sort);
  ...
  public async getAssignedEndpoint({ request, response }: HttpContextContract) {
    const { endpointId, permissionKey } = await request.validate(
      GetAssignedEndpointValidator
    );
    ...
    try {
      ...
      const assignedEndpoint =
        await AssignedEndpointRepository.getAssignedEndpoint(
          endpointId,
          permissionKey
        );
  ...
  public async postAssignedEndpoint({
    request,
    response,
  }: HttpContextContract) {
    try {
  ...
  public async updateAssignedEndpoint({
    request,
    response,
  }: HttpContextContract) {
    try {
      ...
      const assignedEndpoint = await AssignedEndpointRepository.find(
        endpointId,
        permissionKey
      );
      ...
      const updatedPermissionKey =
        data.permissionKey ?? assignedEndpoint.permissionKey;
  ...
  public async deleteAssignedEndpoint({
    request,
    response,
  }: HttpContextContract) {
    try {
      const { endpointId, permissionKey } = await request.validate(
        DeleteAssignedEndpointValidator
      );
      await CheckEndpointExists.validate(endpointId);
      await CheckPermissionExists.validate(permissionKey);
